<?php
    ...
    $result = $connectorDB->createTable($usuarios->tableName, $usuarios->data); //Ejecutar consulta para crear tabla
    ...
    //Iniciar la función crear tabla crearTabla con la información del objeto Eventos
    $result = $connectorDB->createTable($eventos->tableName, $eventos->data);
    ...
    $result =  $connectorDB->newRestriction($eventos->tableName, 'ADD KEY fk_usuarios (fk_usuarios)');
    if( $result == "OK"){
        $response['Index'] = 'OK';
        $response['detalle'] = 'OK';
    }
    ...
    $result =  $connectorDB->newRelation($eventos->tableName, $usuarios->tableName, 'fk_usuarioemail_evento', 'fk_usuarios', 'email'); //nombre de la tabla origen, nomvre tabla destino, nombre de la clave foranea, nombre de la columna origen, nombre de columna destino
    if( $result == "OK"){
